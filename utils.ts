...
export async function readSolidDocument(url:URL | string) {
  try {
    const response = await solidClientAuthentication.fetch(url, {
...
export async function findUserStorage(url:URL|string) :Promise<string>{
  url = url.replace(/#.*$/, '');
...
export function performLogin(loginURLhref:URL|string) {
